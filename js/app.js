...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    // function works for dynamically show icon 
    function iconFunction() {
      const iconArray = [];
      for (var i = 1; i <= ratingRates; i++) {
        iconArray.push('<i class="fas fa-star icon-color"></i>');
      }
      return iconArray;
    }
    // ------ function end -------
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h5><span class="fw-bold text-danger">Average of Rating:</span>${iconFunction()} (${product.rating.rate})</h5 >
